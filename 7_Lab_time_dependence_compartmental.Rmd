...
$S_{t}=S_{t-1}-\lambda_IS_{t-1}$

$I_{t}=I_{t-1}+\lambda_IS_{t-1}$
...
I[1]  <- I0
for (i in 2:11){
  S[t] <- S[t-1]-lambdaI*S[t-1]
  I[t] <- I[t-1]+lambdaI*S[t-1]
}
```
...
(c) Repeat (a) and (b) for different lengths of time (try more than 10 time points, like 50, 100, 200, etc.) How do your results change? 
